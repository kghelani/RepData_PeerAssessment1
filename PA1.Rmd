...
lines(factor(isteps$tinterval),isteps$mean,col="red")
#legend("topleft", c("Mean steps", "Median steps"), lty=c(1) col=c( "red"), bty="n")
...
An average `r maxsteps` steps taken during 5 minute interval starting at `r hours` hours is the maximum average for an interval across all days.
...
hist(stepseachday$tsteps,col=rgb(1,0,1,1/4), main="Steps each day (Original)", xlab="Steps")
```
...
lines(factor(isteps2$tinterval),isteps2$mean,col="blue", lwd=1)
lines(factor(isteps$tinterval),isteps$mean,col="red", lwd=1, type ="s")
legend("topleft", c("Imputed", "Original"), lty=c(1,1), col=c("blue", "red"), bty="n")
axis(1, at=seq(1, length(levels(factor(isteps$tinterval))),12), labels=format(seq(0,23,1), format="%2s"))
```
.
